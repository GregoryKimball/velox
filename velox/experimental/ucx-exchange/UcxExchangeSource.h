...
class UcxExchangeSource
 ...
 private:
  ...
  // Put the received data into the exchange queue and convert any posted
  // receive reservation into queued ownership.
  void enqueue(
      PackedTableWithStreamPtr data,
      uint64_t reservedReceiveBytes = 0);

  bool tryReserveReceiveBytes(std::shared_ptr<DataAndMetadata> data);
  void releaseReceiveBytes(std::shared_ptr<DataAndMetadata> data);
