...
UcxOutputQueue::UcxOutputQueue(
  ...
  if (task_) {
    const auto configuredMaxSize = task_->queryCtx()->queryConfig().get<int64_t>(
        cudf_velox::CudfConfig::kUcxOutputBufferBytes, 0);
    VELOX_USER_CHECK_GE(
        configuredMaxSize,
        0,
        "{} must not be negative",
        cudf_velox::CudfConfig::kUcxOutputBufferBytes);
    maxSize_ = configuredMaxSize > 0
        ? static_cast<uint64_t>(configuredMaxSize)
        : task_->queryCtx()->queryConfig().maxOutputBufferSize();
    continueSize_ = (maxSize_ * kContinuePct) / 100;
    adaptiveFlowControlEnabled_ = task_->queryCtx()->queryConfig().get<bool>(
        cudf_velox::CudfConfig::kUcxAdaptiveFlowControlEnabled, true);
...
bool UcxOutputQueue::checkTransferCapacity(
  ...
  std::lock_guard<std::mutex> l(mutex_);
  if (!adaptiveFlowControlEnabled_) {
    return false;
  }
  ...
  std::vector<ContinuePromise> promises;
  {
    ...
    for (int i = queues_.size(); i <= destination; ++i) {
      ...
      if (kind_ == core::PartitionedOutputNode::Kind::kArbitrary && atEnd_) {
        queues_.back()->enqueueBack(nullptr, /*numRows=*/0);
      }
    }
    auto* queue = queues_[destination].get();
    ...
    if (queue) {
      ...
      if (kind_ == core::PartitionedOutputNode::Kind::kArbitrary &&
        ...
        auto [data, numRows] = std::move(arbitraryBuffer_.front());
