...
class UcxOutputQueue : public std::enable_shared_from_this<UcxOutputQueue> {
 ...
 private:
  ...
  // For arbitrary: shared pool with producer-supplied logical row counts.
  std::deque<
      std::pair<std::shared_ptr<cudf::packed_columns>, vector_size_t>>
      arbitraryBuffer_;
  ...
  bool adaptiveFlowControlEnabled_{true};
