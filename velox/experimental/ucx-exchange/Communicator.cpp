...
#ifdef VELOX_ENABLE_CUDF
  if (!cudf_velox::CudfConfig::getInstance().exchange) {
    return nullptr;
  }
#endif
...
void Communicator::run() {
  ...
  // Intra-node sources can continuously re-enqueue themselves. The bounded
  // dispatcher prevents them from starving deferred callbacks and diagnostics.
  while (running_) {
    try {
      ...
      if (now - lastHeartbeat_ >= std::chrono::seconds(5)) {
        ...
        VLOG(2) << "[COMM-HEARTBEAT] workQueue=" << workQueue_.size()
                ...
                << " workItemsProcessed="
                << std::exchange(workItemsProcessed_, 0)
#ifdef VELOX_ENABLE_CUDF
      ...
      // UCXX progress and callbacks run on worker_'s dedicated progress thread.
      // Yield after the bounded application-state batch so that thread can run.
...
std::shared_ptr<EndpointRef> Communicator::findEndpointRefByHandle(
    ucp_ep_h handle) {
  // Listener registration and active-message callbacks are both serialized
  // by the UCXX progress thread, matching Acceptor's map ownership contract.
