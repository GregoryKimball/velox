...
class Communicator {
 public:
  ...
  /// Returns the initialized singleton, or nullptr before host startup.
  ...
  /// Thread-safe: UCXX callbacks may call this from the progress thread.
 ...
 private:
  ...
  // Processes a bounded batch of state machines on the run() thread.
  void drainWorkQueue();
  ...
  // UCX internals. UCXX callbacks append while the run() thread sweeps, so the
  // vector requires explicit cross-thread synchronization.
