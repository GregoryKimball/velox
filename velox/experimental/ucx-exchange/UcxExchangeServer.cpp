...
void UcxExchangeServer::sendComplete(
  ...
  if (status == UCS_OK) {
    ...
    // getData() moved this chunk from queued to in-flight accounting. Release
    // that ownership only after UCX reports DMA completion; this also wakes
    // producers blocked by the destination transfer window.
...
void UcxExchangeServer::onIntraNodeRetrieveComplete() {
  ...
  if (!intraNodeAtEndPublished_) {
    // The process-local retrieve future is the equivalent of remote DMA
    // completion for producer-side in-flight accounting.
    queueMgr_->releaseInFlightBytes(
        partitionKey_.taskId,
        partitionKey_.destination,
