...
void UcxExchangeSource::enqueueStateEvent(
    std::shared_ptr<UcxExchangeSource> keepAlive,
    std::function<void()> action) {
  if (!keepAlive || !action) {
    return;
  }
  auto wakeSource = keepAlive;
  {
    std::lock_guard<std::mutex> lock(stateEventsMutex_);
    stateEvents_.push_back(
        StateEvent{std::move(keepAlive), std::move(action)});
  }
  communicator_->addToWorkQueue(std::move(wakeSource));
}

void UcxExchangeSource::drainStateEvents() {
...
void UcxExchangeSource::onData(ucs_status_t status, std::shared_ptr<void> arg) {
  ...
  if (status != UCS_OK) {
    ...
    if (shouldPipelineDecompression(*ptr)) {
      ...
      startDecompression(ptr);
      return;
    }

    try {
      decodeAndEnqueue(ptr);
    } catch (const std::exception& e) {
      releaseReceiveBytes(ptr);
...
void UcxExchangeSource::decodeAndEnqueue(std::shared_ptr<DataAndMetadata> ptr) {
  ...
  const uint64_t reservedReceiveBytes = ptr->receiveBytesReserved
      ...
      : 0;
  ptr->receiveBytesReserved = false;
  enqueue(std::move(output), reservedReceiveBytes);
}
