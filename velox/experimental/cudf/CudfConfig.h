...
struct CudfConfig {
  ...
  // Zero preserves QueryConfig::maxOutputBufferSize() as the initial chunk.
  static constexpr const char* kUcxInitialPayloadBytes{
      "cudf.ucx.initial-payload-bytes"};
  // Geometric upper bound relative to the adaptive base transfer window.
  static constexpr const char* kUcxTransferWindowMultiplier{
      "cudf.ucx.transfer-window-multiplier"};
  // Zero preserves QueryConfig::maxOutputBufferSize() as the producer limit.
  static constexpr const char* kUcxOutputBufferBytes{
      "cudf.ucx.output-buffer-bytes"};
  static constexpr const char* kUcxAdaptiveFlowControlEnabled{
      "cudf.ucx.adaptive-flow-control-enabled"};
  // Zero preserves the existing unbounded receiver queue behavior.
  static constexpr const char* kUcxReceivePrefetchBytes{
      "cudf.ucx.receive-prefetch-bytes"};
