...
constexpr int8_t kSpatialMaxDepth = 15;
constexpr int32_t kSpatialLeafCapacity = 256;

__device__ bool readGeometryEnvelope(
    char const* data,
    cudf::size_type len,
    double& minX,
    double& minY,
  ...
  auto const tag = static_cast<uint8_t>(data[0]);
  if (tag == kPointTag) {
    double x = 0;
    double y = 0;
  ...
  return isfinite(minX) && isfinite(minY) && isfinite(maxX) &&
      isfinite(maxY);
}

__device__ bool envelopeIntersects(
    double aMinX,
    double aMinY,
    double aMaxX,
    double aMaxY,
...
struct QuadtreeCandidateGenerator {
  uint32_t const* pointIndices;
  uint32_t const* quadIndices;
  uint32_t const* buildIndices;
  uint32_t const* leafOffsets;
  uint64_t const* pairOffsets;
  int32_t numPairs;

  __device__ thrust::tuple<cudf::size_type, cudf::size_type> operator()(
      uint64_t workIndex) const {
    auto pairIt = thrust::upper_bound(
        thrust::seq,
        pairOffsets,
        pairOffsets + numPairs + 1,
        workIndex);
    auto const pair = static_cast<int32_t>(pairIt - pairOffsets - 1);
    auto const quad = quadIndices[pair];
    auto const local = workIndex - pairOffsets[pair];
    auto const sortedPoint = leafOffsets[quad] + local;
    return thrust::make_tuple(
        static_cast<cudf::size_type>(pointIndices[sortedPoint]),
        static_cast<cudf::size_type>(buildIndices[pair]));
  }
};

struct CandidateEnvelopeIntersects {
  double const* probeMinX;
  double const* probeMinY;
  double const* probeMaxX;
  double const* probeMaxY;
  cudf::bitmask_type const* probeNull;
  double const* buildMinX;
  double const* buildMinY;
  double const* buildMaxX;
  double const* buildMaxY;
  cudf::bitmask_type const* buildNull;

  __device__ bool operator()(
      thrust::tuple<cudf::size_type, cudf::size_type> const& pair) const {
    auto const probe = thrust::get<0>(pair);
    auto const build = thrust::get<1>(pair);
    if ((probeNull && !cudf::bit_is_set(probeNull, probe)) ||
        (buildNull && !cudf::bit_is_set(buildNull, build))) {
      return false;
    }
...
struct CrossCandidateGenerator {
  cudf::size_type numBuild;

  __device__ thrust::tuple<cudf::size_type, cudf::size_type> operator()(
      int64_t index) const {
    return thrust::make_tuple(
        static_cast<cudf::size_type>(index / numBuild),
        static_cast<cudf::size_type>(index % numBuild));
  }
};
...
std::pair<std::unique_ptr<cudf::column>, std::unique_ptr<cudf::column>>
copySpatialCandidates(
    CandidateIterator first,
    CandidateIterator last,
    CandidateEnvelopeIntersects intersects,
    rmm::cuda_stream_view stream,
    rmm::device_async_resource_ref mr) {
  auto const count =
      thrust::count_if(rmm::exec_policy(stream), first, last, intersects);
  CUDF_EXPECTS(
      count <= std::numeric_limits<cudf::size_type>::max(),
      "spatial candidate count exceeds cudf::size_type");
  auto probe = cudf::make_numeric_column(
      cudf::data_type{cudf::type_to_id<cudf::size_type>()},
      static_cast<cudf::size_type>(count),
      cudf::mask_state::UNALLOCATED,
      stream,
      mr);
  auto build = cudf::make_numeric_column(
      cudf::data_type{cudf::type_to_id<cudf::size_type>()},
      static_cast<cudf::size_type>(count),
      cudf::mask_state::UNALLOCATED,
      stream,
  ...
  if (count != 0) {
    auto output = thrust::make_zip_iterator(thrust::make_tuple(
        probe->mutable_view().data<cudf::size_type>(),
        build->mutable_view().data<cudf::size_type>()));
    thrust::copy_if(
        rmm::exec_policy(stream), first, last, output, intersects);
  }
...
GeometryEnvelopes extractGeometryEnvelopes(
  ...
  GeometryEnvelopes result{
      makeColumn(), makeColumn(), makeColumn(), makeColumn()};
  auto* minX = result.minX->mutable_view().data<double>();
  auto* minY = result.minY->mutable_view().data<double>();
  auto* maxX = result.maxX->mutable_view().data<double>();
  auto* maxY = result.maxY->mutable_view().data<double>();
  auto* resultMask = static_cast<cudf::bitmask_type*>(
      result.minX->mutable_view().null_mask());

  cudf::strings_column_view strings(geometry);
  auto chars = strings.chars_begin(stream);
  auto offsets = strings.offsets().begin<cudf::size_type>();
  auto geometryNull = geometry.null_mask();
  auto expand = perRowExpand ? expandBy.data<double>() : nullptr;
  ...
  thrust::for_each_n(
      ...
      [chars,
       ...
       maxY,
       resultMask,
       invalidTypeFlag] __device__(cudf::size_type row) {
        if ((geometryNull && !cudf::bit_is_set(geometryNull, row)) ||
            (perRowExpand && expandNull &&
             !cudf::bit_is_set(expandNull, row))) {
          cudf::clear_bit_unsafe(resultMask, row);
          return;
        }
        auto const begin = offsets[row];
        double x0, y0, x1, y1;
        if (!readGeometryEnvelope(
                chars + begin,
                offsets[row + 1] - begin,
                x0,
                y0,
        ...
        auto const radius =
            fmax(0.0, perRowExpand ? expand[row] : constantExpandBy);
        minX[row] = x0 - radius;
        minY[row] = y0 - radius;
        maxX[row] = x1 + radius;
        maxY[row] = y1 + radius;
  ...
  auto const nullCount =
      cudf::null_count(result.minX->view().null_mask(), 0, size, stream);
  result.minX->set_null_count(nullCount);
  auto const maskBytes = cudf::bitmask_allocation_size_bytes(size);
  for (auto* column :
       {result.minY.get(), result.maxX.get(), result.maxY.get()}) {
    CUDF_CUDA_TRY(cudaMemcpyAsync(
        column->mutable_view().null_mask(),
        result.minX->view().null_mask(),
        maskBytes,
        cudaMemcpyDeviceToDevice,
        stream.value()));
    column->set_null_count(nullCount);
  }
  return result;
}
...
GeometryEnvelopeGrid buildGeometryEnvelopeGrid(
  ...
  return GeometryEnvelopeGrid{std::move(buildEnvelopes)};
}
...
queryGeometryEnvelopeGrid(
  ...
  auto const numBuild = grid.envelopes.minX->size();
  if (numProbe == 0 || numBuild == 0) {
    return emptySpatialPairs();
  }
  ...
  // cuSpatial's point quadtree is exact for point probes. Preserve correctness
  // for other geometry shapes by falling back to envelope refinement.
  auto const nonPointCount = thrust::count_if(
      rmm::exec_policy(stream),
      thrust::counting_iterator<cudf::size_type>(0),
      thrust::counting_iterator<cudf::size_type>(numProbe),
      [pMinX, pMinY, pMaxX, pMaxY, pNull] __device__(
          cudf::size_type row) {
        return (!pNull || cudf::bit_is_set(pNull, row)) &&
            (pMinX[row] != pMaxX[row] || pMinY[row] != pMaxY[row]);
      });
  if (nonPointCount != 0) {
    auto const total =
    ...
    CUDF_EXPECTS(
        ...
        "spatial envelope cross product exceeds cudf::size_type");
    auto candidates = thrust::make_transform_iterator(
        thrust::counting_iterator<int64_t>(0),
        CrossCandidateGenerator{numBuild});
    return copySpatialCandidates(
        candidates, candidates + total, intersects, stream, mr);
  }

  constexpr double kLarge = 1.0e300;
  auto reduceCoordinate = [&](double const* values, bool findMin) {
    if (findMin) {
      return thrust::transform_reduce(
          rmm::exec_policy(stream),
          thrust::counting_iterator<cudf::size_type>(0),
          thrust::counting_iterator<cudf::size_type>(numProbe),
          [values, pNull] __device__(cudf::size_type row) -> double {
            return pNull && !cudf::bit_is_set(pNull, row)
                ? 1.0e300
                : values[row];
          },
  ...
  double minX = reduceCoordinate(pMinX, true);
  double minY = reduceCoordinate(pMinY, true);
  double maxX = reduceCoordinate(pMaxX, false);
  double maxY = reduceCoordinate(pMaxY, false);
  if (!(minX <= maxX) || !(minY <= maxY)) {
    return emptySpatialPairs();
  }

  auto span = std::max(maxX - minX, maxY - minY);
  auto const padding = std::max(span * 1.0e-6, 1.0e-9);
  minX -= padding;
  minY -= padding;
  maxX += padding;
  maxY += padding;
  span = std::max(maxX - minX, maxY - minY);
  auto const scale =
      span / static_cast<double>((1u << kSpatialMaxDepth) - 2u);
  auto points = thrust::make_transform_iterator(
      thrust::counting_iterator<cudf::size_type>(0),
      [pMinX, pMinY, pNull, minX, minY] __device__(
          cudf::size_type row) -> cuspatial::vec_2d<double> {
        if (pNull && !cudf::bit_is_set(pNull, row)) {
          return {minX, minY};
        }
        return {pMinX[row], pMinY[row]};
      });
  auto [pointIndices, quadtree] = cuspatial::quadtree_on_points(
      points,
      points + numProbe,
      cuspatial::vec_2d<double>{minX, minY},
      cuspatial::vec_2d<double>{maxX, maxY},
      scale,
      kSpatialMaxDepth,
      kSpatialLeafCapacity,
      stream,
      mr);

  auto boxes = thrust::make_transform_iterator(
      thrust::counting_iterator<cudf::size_type>(0),
      [bMinX, bMinY, bMaxX, bMaxY, bNull, minX, minY, span] __device__(
          cudf::size_type row) -> cuspatial::box<double> {
        if (bNull && !cudf::bit_is_set(bNull, row)) {
          auto const outside = minX - span * 2.0;
          return {{outside, outside}, {outside, outside}};
        }
        return {{bMinX[row], bMinY[row]}, {bMaxX[row], bMaxY[row]}};
      });
  auto [buildIndices, quadIndices] =
      cuspatial::join_quadtree_and_bounding_boxes(
          cuspatial::point_quadtree_ref{quadtree},
          boxes,
          boxes + numBuild,
          cuspatial::vec_2d<double>{minX, minY},
          scale,
          ...
          stream,
          mr);
  auto const numPairs = static_cast<int32_t>(buildIndices.size());
  if (numPairs == 0) {
    return emptySpatialPairs();
  }

  rmm::device_uvector<uint64_t> pairOffsets(
      static_cast<size_t>(numPairs) + 1, stream, mr);
  pairOffsets.set_element_to_zero_async(0, stream);
  auto pairLengths = thrust::make_transform_iterator(
      quadIndices.begin(),
      [lengths = quadtree.length.data()] __device__(
          uint32_t quad) -> uint64_t {
        return static_cast<uint64_t>(lengths[quad]);
      });
  thrust::inclusive_scan(
      rmm::exec_policy(stream),
      pairLengths,
      pairLengths + numPairs,
      pairOffsets.begin() + 1);
  auto const numCandidates = pairOffsets.back_element(stream);
  if (numCandidates == 0) {
    return emptySpatialPairs();
  }

  QuadtreeCandidateGenerator generator{
      pointIndices.data(),
      quadIndices.data(),
      buildIndices.data(),
      quadtree.offset.data(),
      pairOffsets.data(),
      numPairs};
  auto candidates = thrust::make_transform_iterator(
      thrust::counting_iterator<uint64_t>(0), generator);
  return copySpatialCandidates(
      candidates,
