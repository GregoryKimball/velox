 ...
 public:
  ...
  RowVectorPtr getOutput() override {
    ...
    auto output = std::make_shared<RowVector>(
        pool(), node_->outputType(), nullptr, input_->size(), std::move(children));
    input_.reset();
    return output;
  }
...
class IndexBuildOperator final : public exec::Operator {
 public:
  ...
  void addInput(RowVectorPtr input) override {
    ...
    const auto* ids = input->childAt(
                               buildNode_->outputType()->getChildIdx(
                                   buildNode_->idColumn()))
...
class IndexSearchOperator final : public exec::Operator {
 public:
  ...
  RowVectorPtr getOutput() override {
    ...
    const auto* queryIds =
        input_->childAt(queryType->getChildIdx(node_->queryIdColumn()))
            ->as<SimpleVector<int64_t>>();
    ...
    for (vector_size_t row = 0; row < input_->size(); ++row) {
      ...
      const auto vector = embeddingAt(arrays, row);
      it->second.index->search(
          1, vector.data(), node_->topK(), distances.data(), labels.data());
      for (int32_t rank = 0; rank < node_->topK(); ++rank) {
        ...
        if (node_->maxDistance() &&
            distances[rank] > *node_->maxDistance()) {
          continue;
        }
        outputQueryIds.push_back(queryIds->valueAt(row));
...
std::unique_ptr<exec::Operator> FaissPlanNodeTranslator::toOperator(
  ...
  if (auto assign =
          std::dynamic_pointer_cast<const AssignClustersNode>(node)) {
...
void registerFaiss() {
  ...
  exec::Operator::registerOperator(
      std::make_unique<FaissPlanNodeTranslator>());
}
